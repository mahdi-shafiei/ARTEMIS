...
def get_latent_trajectories(train_dataset,val_latent, tps, tr_model, ts, val_data_init = None, t_0_orig=0, max_size=0, test=False):

    if val_data_init is None:
    ...
    return pred_trajectories, pred_trajectories_alive, np.array(timepoints), np.array(timepoints_alive), statuses, birth_statuses
...
def get_reconstructed_trajectory(pred_trajectories, val_data, vae_input_dim, vae_hidden_dim, vae_latent_dim, tr_model, timepoints):
    ...
    dec_init_params = dec.init(next(rng_seq), z=np.zeros((val_data.shape[0], vae_latent_dim+tr_model.training_setup.vae_t_dim)))

    for key in tr_model.vae_params.keys():
        if "dec" in key:
            dec_init_params[key.split("~/")[1]] = tr_model.vae_params[key]
            
    t_emb = get_timestep_embedding(np.array(timepoints), tr_model.training_setup.vae_t_dim)
...
def get_predictions(train_data, val_data, train_tps, val_tps,
                    ...
                    num_simulations=5, t_0_orig=0, val_data_init=None):
    ...
    predictions_all["statuses"] = {}

    for i in range(num_simulations):

        # heldout time point
        pred_trajectories, pred_trajectories_alive, timepoints, timepoints_alive, statuses, birth_statuses = get_latent_trajectories(train_dataset,train_latent, tps, tr_model, ts, val_data_init=val_data_init, max_size=2000, t_0_orig=t_0_orig)
        ...
        predictions_all["statuses"][i] = (statuses, birth_statuses)
